...
##Ahora tendriamos 4 variables (sexo, edad, preguntas, respuestas)
```{r}
...
```
#---------------------------------------------------------------------------------------------
#Cantidad por Genero. Rango entre 15 y 20
```{r}
...
numero_por_genero_entre15y20 %>% ggplot(aes(x = Sexo, y = Cantidad)) + 
  ...
  theme_minimal()
```
#Cantidad por Genero. rango entre 35 y 40
```{r}
numero_por_genero_entre35y40 <- poblacion_entre35y40 %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())
numero_por_genero_entre35y40 %>% ggplot(aes(x = Sexo, y = Cantidad)) + 
  geom_col(colour = "black",
           fill = "#1380A1",
           width = 0.5) + 
  labs(x = "Sexo",
  ...
  theme_minimal()
```
#Cantidad por género. Rango entre 55 y 60
```{r}
numero_por_genero_entre55y60 <- poblacion_entre55y60 %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())
numero_por_genero_entre55y60 %>% ggplot(aes(x = Sexo, y = Cantidad)) + 
  geom_col(colour = "black",
           fill = "#1380A1",
           width = 0.5) + 
  labs(x = "Sexo",
  ...
  theme_minimal()
```
#Grafico respuesta ganadora -Rango entre 15 y 20 años
```{r}
...
```
#Grafico respuesta ganadora -Rango entre 35 y 40 años
```{r}
...
```
#Grafico respuesta ganadora -Rango entre 55 y 60 años
```{r}
...
pensamiento_suicida %>% ggplot(aes(x = Respuestas, y = Cantidad)) +
  ...
  theme_minimal()
```
#IDEACION SUICIDA EN RANGO DE EDADES
#Cantidad en funcion del genero para el rango entre 10 y 20 anios
```{r}
respuesta_5_en_todas <- datos_totales_Tidy %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())

respuesta_5_en_todas %>% ggplot(aes(x = Sexo, y = Cantidad, color = Sexo, fill = Sexo)) +
  geom_dots() +
  labs(title = "Poblacion total según género- Ideación suicida") +
  theme_minimal()
```
...
#Cantidad en funcion del genero para el rango entre 10 y 20 anios
```{r}
respuesta_5_rango10y20 <- datos_totales_Tidy %>% 
  filter(Edad >= 10 & Edad <= 20) %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())

respuesta_5_rango10y20 %>% ggplot(aes(x = Sexo, y = Cantidad, color = Sexo, fill = Sexo)) +
  geom_dots() +
  labs(title = "Poblacion segun genero entre 10 y 20 años") +
  theme_minimal()
```
```{r}
respuesta_5_rango10y20 <- datos_totales_Tidy %>% 
  ...
  group_by(Preguntas) %>% 
  summarise(Cantidad = n())
respuesta_5_rango10y20

respuesta_5_rango10y20$Preguntas <- factor(respuesta_5_rango10y20$Preguntas, levels = c("PocoInteres", "SensacionDeDepresion","ProblemasDeSuenio","SensacionDeCansancio", "PocoApetito", "SensacionDeFracaso", "ProblemasDeConcentracion", "AccionarLento", "PensamientosSuicidas")) 

respuesta_5_rango10y20 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) +
  ggtitle("Respuesta máxima en todas las preguntas- Rango entre 10 y 20 años") +
  geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9')) +
  scale_fill_discrete(labels = c('1- Poco Interés','2- Sensación de Depresión','3- Problemas de Sueño', '4- Sensación de Cansancio', '5- Poco Apetito', '6- Sensación de Fracaso', '7- Problemas de Concentración', '8- Accionar Lento', '9- Pensamientos Suicidas')) +
  geom_text(aes(label = Cantidad),  vjust = 1.5, colour = "white")
```
##Cantidad en funcion del genero para el rango entre 30 y 40 anios
```{r}
respuesta_5_rango30y40 <- datos_totales_Tidy %>% 
  filter(Edad >= 30 & Edad <= 40) %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())

respuesta_5_rango30y40 %>% ggplot(aes(x = Sexo, y = Cantidad, color = Sexo, fill = Sexo)) +
  geom_dots() +
  labs(title = "Poblacion segun genero entre 30 y 40 años") +
  theme_minimal()
```
...
respuesta_5_rango30y40 <- datos_totales_Tidy %>% 
  ...
  arrange(Preguntas) %>%
  group_by(Preguntas) %>% 
  summarise(Cantidad = n()) #falta el calculo de freq relativa... 
respuesta_5_rango30y40

respuesta_5_rango30y40$Preguntas <- factor(respuesta_5_rango30y40$Preguntas, levels = c("PocoInteres", "SensacionDeDepresion","ProblemasDeSuenio","SensacionDeCansancio", "PocoApetito", "SensacionDeFracaso", "ProblemasDeConcentracion", "AccionarLento", "PensamientosSuicidas")) 

  
respuesta_5_rango30y40 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + ggtitle("Respuesta máxima en todas las preguntas- Rango entre 30 y 40 años") +
  geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9')) +
  scale_fill_discrete(labels = c('1- Poco Interés','2- Sensación de Depresión','3- Problemas de Sueño', '4- Sensación de Cansancio', '5- Poco Apetito', '6- Sensación de Fracaso', '7- Problemas de Concentración', '8- Accionar Lento', '9- Pensamientos Suicidas')) +
  geom_text(aes(label = Cantidad),  vjust = 1.5, colour = "white")
```
##Cantidad en funcion del genero para el rango entre 50 y 60 anios
```{r}
respuesta_5_rango50y60 <- datos_totales_Tidy %>% 
  filter(Edad >= 50 & Edad <= 60) %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())

respuesta_5_rango50y60 %>% ggplot(aes(x = Sexo, y = Cantidad, color = Sexo, fill = Sexo)) +
  geom_dots() +
  labs(title = "Poblacion segun genero entre 50 y 60 años") +
  theme_minimal()
```
...
respuesta_5_rango50y60 <- datos_totales_Tidy %>% 
  ...
  group_by(Preguntas) %>% 
  summarise(Cantidad = n()) #falta el calculo de freq relativa... 
respuesta_5_rango50y60

respuesta_5_rango50y60$Preguntas <- factor(respuesta_5_rango50y60$Preguntas, levels = c("PocoInteres", "SensacionDeDepresion","ProblemasDeSuenio","SensacionDeCansancio", "PocoApetito", "SensacionDeFracaso", "ProblemasDeConcentracion", "AccionarLento", "PensamientosSuicidas")) 

  
respuesta_5_rango50y60 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + ggtitle("Respuesta máxima en todas las preguntas- Rango entre 50 y 60 años") +
  geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9')) +
  scale_fill_discrete(labels = c('1- Poco Interés','2- Sensación de Depresión','3- Problemas de Sueño', '4- Sensación de Cansancio', '5- Poco Apetito', '6- Sensación de Fracaso', '7- Problemas de Concentración', '8- Accionar Lento', '9- Pensamientos Suicidas')) +
  geom_text(aes(label = Cantidad),  vjust = 1.5, colour = "white")
```
...
#Relacion transitiva: Sensacion de depresion ->> Sensacion de fracaso ->> Pensamientos suicidas
 la diferencia es que este análisis es bivariado mientras que el de redes es multivariado.donde se hace una estimación de redes no direccionales a partir de correlaciones parciales, básicamente, permite estimar en un modelo único las correlaciones parciales entre un conjunto de indicadores dados
...
#FALTA HACER UN MEJOR GRAFICO DE ESTO

```

#Matriz de correlacion entre Sensacion de depresion ->> Sensacion de fracaso ->> Pensamientos suicidas y Edad
```{r}
#FILTRAR LAS COLUMNAS DEPRESION, FRACASO Y SUICIDIO Y CORRELACIONAR CON LA EDAD
...
#------------------------------------------------------------------------------------------
#INTENTO DE PRUEBA t Y DATOS DESCRIPTIVOS
Se agrega columna Suicide-Extrem ya categorizada en 0 para suicida y 1 no suicida.
para muestras independientes donde la variable suicidio como grupo (sí vs no), y las otras preguntas como las variables a comparar
```{r}
rm(list = ls())
...
datos_Tidy <- Base %>% 
  pivot_longer(c(phq1,phq2,phq3,phq4,phq5,phq6,phq7,phq8),names_to = "Preguntas", values_to = "Respuestas") %>% 
   drop_na()
```

```{r}
#Prueba t y datos descriptivos


t.test(datos_Tidy$Respuestas ~ datos_Tidy$Suicide_extrem)
```

```{r}
datos_Tidy %>% 
  group_by(Suicide_extrem) %>% 
  summarise(mean(Respuestas,  na.rm = TRUE))

datos_Tidy %>%
group_by(Suicide_extrem, Preguntas) %>% 
skim() 
```

```{r}
#Graficas para el total de las preguntas sin diferenciar
datos_Tidy %>%
 mutate(Suicide_extrem = as.factor(Suicide_extrem)) %>% 
ggplot(aes(x = Respuestas)) +
  geom_histogram() +
...
```{r}
#Graficas diferenciando las preguntas
...
permite estimar en un modelo único las correlaciones parciales entre un conjunto de indicadores dados
## 1.  Bases de Datos modificada. 
