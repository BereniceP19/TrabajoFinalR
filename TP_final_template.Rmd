...
```
#---------------------------------------------------------------------------------------------
#RESPUESTA GANADORA EN CADA PREGUTNA PARA DISTINTOS RANGOS DE EDADES
#entre 15 y 20 anios
...
```
#entre 15 y 20 anios
#Cantidad por Genero. rango entre 55 y 60
```{r}
...
numero_por_genero_entre55y60 %>% ggplot(aes(x = Sexo, y = Cantidad)) + 
  ...
  theme_minimal()
```
#grafico respuesta ganadora en este rango de edades entre 15 y 20
```{r}
...
```
#grafico respuesta ganadora en este rango de edades entre 35 y 40
```{r}
...
```
#grafico respuesta ganadora en este rango de edades entre 55 y 60
```{r}
...
respuesta_5_en_todas %>% ggplot(aes(x = Sexo, y = Cantidad, color = Sexo, fill = Sexo)) +
  geom_dots() +
  labs(title = "Poblacion segun genero total") +
  theme_minimal()
```
...
respuesta_5_en_todas %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9'))
```
...
respuesta_5_rango10y20 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9'))
```
...
respuesta_5_rango30y40 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9'))
```
...
respuesta_5_rango50y60 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9'))
```
...
## 1.  Bases de Datos modificada. Se agrega columna Suicide-Extrem ya categorizada.
