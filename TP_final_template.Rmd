...
fill_para_grafico <- respuestas_por_pregunta[1] %>% group_by(Respuestas) %>% summarise()
fill_para_grafico
...
```
#Solo Respuesta 5 en todas las preguntas
```{r}
...
```{r}
pensamiento_suicida <- datos_totales_Tidy %>% filter(Preguntas == "P9") %>%
  group_by(Respuestas) %>% summarise(Cantidad = n())
...
```{r}
poblacion_entre_20y25 <- datos_totales_Tidy %>% filter(Edad >= 20 & Edad <= 25) 
...
#----------------------------------------------------------------------------------------------------
#Primeros Graficos
#grafico pensamientos suicidas 5 en funcion del sexo . TIRA ERROR!!!!
```{r}
