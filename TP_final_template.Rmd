...
### Conversión de la tabla a tidy. Las preguntas ahora aparecen en una sola columna llamada "Preguntas", y los valores de respuesta aparecen en una columna asociada llamada "Respuestas". Ahora tendriamos 4 variables (sexo, edad, preguntas, respuestas)
```{r}
...
```
###Grafico General - Respuestas para cada pregunta
```{r}
...
##Cantidad por Genero.En esta etapa se va a hacer foco en tres grupos etarios que consideramos más relevantes debido a las respuestas otorgadas: 15-20 años, 35-40 años, y 55-60 años.
...
numero_por_genero_entre55y60 %>% ggplot(aes(x = Sexo, y = Cantidad/9)) + 
  ...
  theme_minimal()
```
##Respuestas por pregunta -Rango entre 15 y 20 años
```{r}
...
```
#Respuestas por pregunta -Rango entre 35 y 40 años
```{r}
...
##Filtro para Respuesta 9 (Ideación Suicida) total
```{r}
...
pensamiento_suicida %>% ggplot(aes(x = Respuestas, y = Cantidad)) +
  ...
  labs(x = "Respuestas",
       y = "Cantidad de Respuestas",
       title = "Respuestas para ideación suicida- General") + 
  theme_minimal()
```
##Ideación suicida según género -Población total
```{r}
respuesta_5_en_todas <- datos_totales_Tidy %>% 
  group_by(Sexo) %>% 
  summarise(Cantidad = n())

respuesta_5_en_todas %>% ggplot(aes(x = Sexo, y = Cantidad)) +
  geom_col(width = 0.5, color = "blue") +
  labs(x = "Sexo",
       y = "Cantidad",
       title = "Ideación suicida según género- Población total") +
  theme_minimal()
  
```
##Frecuencia Relativa de respuesta 5 en todas las preguntas -General
```{r}
respuesta_5_en_todas <- datos_totales_Tidy %>% 
  filter(Respuestas == 5) %>%
  arrange(Preguntas) %>%
...
## Análisis ideación suicida según la edad
###Rango entre 10 y 20 años
```{r}
...
respuesta_5_rango10y20 %>% ggplot(aes(x = Sexo, y = Cantidad)) +
  geom_col(width = 0.5, color = "blue") +
  labs(x = "Sexo",
       y = "Cantidad",
       title = "Ideación suicida segun género -10 a 20 años") +
  theme_minimal()
...
respuesta_5_rango10y20 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) +
  ggtitle("Respuesta máxima en todas las preguntas- 10 a 20 años") +
  geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9')) +
  scale_fill_discrete(labels = c('1- Poco Interés','2- Sensación de Depresión','3- Problemas de Sueño', '4- Sensación de Cansancio', '5- Poco Apetito', '6- Sensación de Fracaso', '7- Problemas de Concentración', '8- Accionar Lento', '9- Pensamientos Suicidas')) +
  geom_text(aes(label = Cantidad),  vjust = 1.5, colour = "white")
```
###Rango entre 30 y 40 años
```{r}
...
respuesta_5_rango30y40 %>% ggplot(aes(x = Sexo, y = Cantidad)) +
  geom_col(width = 0.5, color = "blue") +
  labs(x = "Sexo",
       y = "Cantidad",
       title = "Ideación suicida según género-30 a 40 años") +
  theme_minimal()
...
respuesta_5_rango30y40 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + ggtitle("Respuesta máxima en todas las preguntas- 30 a 40 años") +
  geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9')) +
  scale_fill_discrete(labels = c('1- Poco Interés','2- Sensación de Depresión','3- Problemas de Sueño', '4- Sensación de Cansancio', '5- Poco Apetito', '6- Sensación de Fracaso', '7- Problemas de Concentración', '8- Accionar Lento', '9- Pensamientos Suicidas')) +
  geom_text(aes(label = Cantidad),  vjust = 1.5, colour = "white")
```
###Rango entre 50 y 60 años
```{r}
...
respuesta_5_rango50y60 %>% ggplot(aes(x = Sexo, y = Cantidad)) +
  geom_col(width = 0.5, color = "blue") +
  labs(x = "Sexo",
       y = "Cantidad",
       title = "Ideación suicida según género- 50 a 60 años") +
  theme_minimal() 
```
...
respuesta_5_rango50y60 %>% group_by(Preguntas) %>%   summarise(Cantidad = sum(Cantidad)) %>%   ggplot(aes(x = Preguntas, y=Cantidad, fill=Preguntas)) + ggtitle("Respuesta máxima en todas las preguntas-50 a 60 años") +
  geom_col() + scale_x_discrete(labels = c('1','2','3', '4', '5', '6', '7', '8', '9')) +
  scale_fill_discrete(labels = c('1- Poco Interés','2- Sensación de Depresión','3- Problemas de Sueño', '4- Sensación de Cansancio', '5- Poco Apetito', '6- Sensación de Fracaso', '7- Problemas de Concentración', '8- Accionar Lento', '9- Pensamientos Suicidas')) +
  geom_text(aes(label = Cantidad),  vjust = 1.5, colour = "white")
```
...
ggplot(aes(x = Respuestas, fill = Suicide_extrem)) +
    geom_histogram() +
