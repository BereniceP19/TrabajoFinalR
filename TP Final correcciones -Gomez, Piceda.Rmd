...
```
(VOLVER A REVISAR SI AHORA QUEDO OK) NOTA: Esta enumeración quedó mal en el html y se hace muy difícil de leer.
```

```{r}
rm(list = ls())
datos_totales <- read.csv("Datos_Reduccion.csv", sep = ";", dec = ".", header = TRUE)
matriz <- datos_totales[,3:11]
cor <- cor(matriz)
cor2 <- chart.Correlation(matriz)
```

Se observó una relación transitiva entre los sítomas Sensacion de depresion ->> Sensacion de fracaso ->> Pensamientos suicidas

(NOTA: SUMARIA ESTO, PERO FALTA REVISAR)
...
```
(FALTA REVISAR DESDE CORRELACIONES MAS QUE NADA) NOTA: Esta explicación está bien y el análisis y los gráficos también. Sin embargo, falta interpretación tanto de los gráficos de redes como de las comparaciones entre redes.
